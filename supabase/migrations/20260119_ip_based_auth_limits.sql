-- Create ip_usage table for strict IP-based usage limits
-- All authenticated users from the same IP share the same daily quota
...
CREATE INDEX IF NOT EXISTS idx_ip_usage_last_reset ON public.ip_usage(last_reset);
...
CREATE POLICY "Service role full access to ip_usage" ON public.ip_usage
  FOR ALL USING (true) WITH CHECK (true);

-- Update the process_authenticated_request function to use IP-based tracking
...
BEGIN
  ...
  -- Log the usage (still track by user for analytics)
...
-- Update increment function as well
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO ip_usage (ip_address, request_count, last_reset, created_at)
    VALUES (v_safe_ip, 1, v_now, v_now)
    RETURNING * INTO v_ip_record;
  ...
  ELSE
    ...
    IF v_hours_since_reset >= p_reset_interval_hours THEN
      -- Reset counter and set count to 1
      UPDATE ip_usage
      SET request_count = 1,
          last_reset = v_now
      WHERE ip_address = v_safe_ip;
    ...
    ELSE
      ...
      v_remaining := p_daily_limit - (v_ip_record.request_count + 1);
    END IF;
  END IF;
